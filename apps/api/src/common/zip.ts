/**
 * Row 117: a tiny ZIP writer (store method, no compression) so exports need no
 * extra dependency. Files are UTF-8; timestamps are "now".
 */
const CRC_TABLE = (() => {
...
export function zipFiles(files: { name: string; data: string | Buffer }[]) {
  const locals: Buffer[] = [];
