...
export const invoices = pgTable(
  "invoices",
  {
    ...
    /** Row 157: set when this invoice was generated by a billing schedule. */
    scheduleId: uuid("schedule_id").references((): AnyPgColumn => invoiceSchedules.id, { onDelete: "set null" }),
  ...
  (t) => [
    ...
    index("invoices_schedule_idx").on(t.scheduleId),
  ],
);
...
export const invoicesRelations = relations(invoices, ({ one, many }) => ({
  ...
  schedule: one(invoiceSchedules, { fields: [invoices.scheduleId], references: [invoiceSchedules.id] }),
...
/* ------------------------------------------------------------------ *
 * Row 157: recurring & subscription invoices
 * ------------------------------------------------------------------ */
export const scheduleKind = pgEnum("invoice_schedule_kind", ["recurring", "subscription"]);
export const scheduleUnit = pgEnum("invoice_schedule_unit", ["week", "month", "year"]);
export const scheduleStatus = pgEnum("invoice_schedule_status", ["active", "paused", "ended"]);
...
/**
 * A template invoice plus a cadence. The sweep turns it into a real invoice
 * each time `next_run_at` arrives; `every` × `unit` is the gap (weekly = 1 week,
 * monthly = 1 month, quarterly = 3 months, yearly = 1 year, custom = anything).
 */
export const invoiceSchedules = pgTable(
  "invoice_schedules",
  {
    ...
    organizationId: uuid("organization_id")
      ...
      .references(() => organizations.id, { onDelete: "cascade" }),
    name: varchar("name", { length: 255 }).notNull(),
    kind: scheduleKind("kind").notNull().default("recurring"),
    status: scheduleStatus("status").notNull().default("active"),
    /* --- what each invoice looks like --- */
    title: varchar("title", { length: 255 }).notNull(),
    companyId: uuid("company_id").references(() => companies.id, { onDelete: "set null" }),
    contactId: uuid("contact_id").references(() => contacts.id, { onDelete: "set null" }),
    projectId: uuid("project_id").references(() => projects.id, { onDelete: "set null" }),
    currency: varchar("currency", { length: 8 }).notNull().default("USD"),
    items: jsonb("items").$type<ScheduleItem[]>().notNull().default([]),
    taxRate: doublePrecision("tax_rate").notNull().default(0),
    discountPercent: doublePrecision("discount_percent").notNull().default(0),
    notes: text("notes"),
    /** Net terms: due date = issue date + this many days. */
    ...
    /** Day-of-month anchor for month/year cadences so the 31st doesn't drift to the 28th forever. */
  ...
  (t) => [
    index("invoice_schedules_org_status_idx").on(t.organizationId, t.status),
    index("invoice_schedules_next_idx").on(t.nextRunAt),
    index("invoice_schedules_company_idx").on(t.companyId),
  ],
);

export const invoiceSchedulesRelations = relations(invoiceSchedules, ({ one, many }) => ({
  company: one(companies, { fields: [invoiceSchedules.companyId], references: [companies.id] }),
